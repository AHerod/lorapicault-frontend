<template lang="pug">
  section(class="md:px-0 pb-14 pt-10 md:py-28").bg-primary.text-white
    ...
    div(class="grid grid-cols-4 gap-4 md:gap-8 px-4 md:px-8 pt-6 md:px-24")
      cool-light-box(:items="foto" :index="index" @close="index = null" :autoplay="true" :useZoomBar="true")
      div(:style="`height: ${height}px`" class="shadow-2xl relative w-full" v-for="(image,imageIndex) in foto" :key="imageIndex"  @click="index = imageIndex" :class=`[ image.size === 1 ? 'col-span-2 md:col-span-1' : 'col-span-4 md:col-span-2']`)
